...
Reactive.prototype.checkModel = function (model) {
  ...
  fields.forEach(function (field) {
    if (!util.isFunction(model, field)) {
      var warn = console ? console.warn : null // eslint-disable-line
      if (typeof warn === 'function') {
        console.warn(field + ' is not a function on model') // eslint-disable-line
      }
    }
  })
}
...
Reactive.prototype._bind = function (model, root) {
  ...
  util.walk(root, function (node, next, single) {
    ...
    if (node.nodeType === 3) {
      ...
      var hasFormat = node.hasAttribute('data-format')
      if (hasFormat && !single) throw new Error('Can\'t parse data-format for element with child element ' + node.textContent)
      ...
      if (single && util.hasInterpolation(text) && !hasFormat) {
