...
function Reactive(el, model, option) {
  ...
  this.filters = util.assign({}, option.filters || {})
  util.assign(this.filters, filters)
  this.model = model
  this.el = el
 ...
 * @api public
 */
var getDelegate = Reactive.prototype.getDelegate = function (name) {
...
Reactive.generateConfig = function (el, model, delegate, bindings) {
  ...
  bindings = bindings || {}
  // a fake one
  var reactive = {el: el, model: model, delegate: delegate, bindings: bindings, getDelegate: getDelegate}
...
Reactive.createBinding = function (name, fn) {
  if (bindings[name]) throw new Error('Global binding name [' + name+ '] already in use')
  bindings[name] = fn
}
